...
| # | Metric | Source | Value |
|---|--------|--------|-------|
| 1 | Random transactions run, 0 mismatches | `tb_async_fifo.sv` METRIC prints (`TOTAL_WRITES_ACCEPTED`, `SCOREBOARD_MISMATCHES`) | writes = [ ], mismatches = [ ] |
| 2 | Functional coverage % | `COVERAGE_PCT` METRIC print | [ ] % |
| 3 | Max occupancy reached / DEPTH | `MAX_OCCUPANCY` print + Python cross-check | [ ] / 16 |
| 4 | Backpressure watermark accuracy | `ALMOST_FULL_ASSERT_AT_OCC` / `..._DEASSERT_AT_OCC` prints | assert @ [ ], deassert @ [ ] (expect 14 / 10) |
...
```
METRIC: TOTAL_WRITES_ACCEPTED = 
METRIC: TOTAL_READS_VALID = 
METRIC: SCOREBOARD_MISMATCHES = 
METRIC: MAX_OCCUPANCY = 
METRIC: FULL_ASSERT_COUNT = 
METRIC: ALMOST_FULL_ASSERT_COUNT = 
METRIC: EMPTY_ASSERT_COUNT = 
METRIC: COVERAGE_PCT = 
```
